...
--
-- Name: hiring_age_view; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.hiring_age_view AS
 SELECT employees.employeeid,
    employees.firstname,
    employees.lastname,
    employees.hiredate,
    employees.birthdate,
    (date_part('year'::text, employees.hiredate) - date_part('year'::text, employees.birthdate)) AS hiring_age
   FROM public.employees
  ORDER BY (date_part('year'::text, employees.hiredate) - date_part('year'::text, employees.birthdate));


ALTER TABLE public.hiring_age_view OWNER TO postgres;
...
CREATE TABLE public.orders (
    ...
    requierddate timestamp without time zone,
    shippeddate timestamp without time zone,
    shipvia integer,
    freight numeric,
    shipname character varying(250),
    shipaddress character varying(250),
...
--
-- Name: order_amount_by_country_view; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.order_amount_by_country_view AS
 SELECT customers.country,
    sum((order_details.unitprice * (order_details.quantity)::numeric)) AS order_amount
   FROM ((public.order_details
     JOIN public.orders ON ((order_details.orderid = orders.orderid)))
     JOIN public.customers ON (((orders.customerid)::text = (customers.customerid)::text)))
  GROUP BY customers.country
  ORDER BY (sum((order_details.unitprice * (order_details.quantity)::numeric))) DESC;


ALTER TABLE public.order_amount_by_country_view OWNER TO postgres;
...
CREATE TABLE public.products (
    ...
    supplierid integer,
    categoryid integer,
    quantityperunit character varying(300),
    unitprice numeric,
...
--
-- Name: product_orders_by_country_view; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.product_orders_by_country_view AS
 SELECT count(order_details.orderid) AS n_orders,
    products.productname,
    orders.shipcountry
   FROM ((public.order_details
     JOIN public.orders ON ((order_details.orderid = orders.orderid)))
     JOIN public.products ON ((order_details.productid = products.productid)))
  GROUP BY products.productname, orders.shipcountry
  ORDER BY (count(order_details.orderid));


ALTER TABLE public.product_orders_by_country_view OWNER TO postgres;
...
CREATE TABLE public.regions (
    ...
    regiondescription character varying(250)
);
...
COPY public.orders (orderid, customerid, employeeid, orderdate, requierddate, shippeddate, shipvia, freight, shipname, shipaddress, shipcity, shipregion, shippostalcode, shipcountry) FROM stdin;
